...
const { logProgress, logSuccess, logError, logWarning, log, logInfo } = require('./logging');
const { PATH_DISCOVERY, PATH_NEW_PEOPLE, PATH_RECENTLY_ONLINE, PATH_MATCHING_INTEREST, PATH_MATCHING_SEARCH_CRITERIA } = require('./path');
                    ...
                    if(element.matchProfile.pictures.length > 0){
                        await element.matchProfile.pictures.forEach(async picItem => {
                            if(likesData.includes(picItem.id)){
                                logInfo(`Horrayy! finding matched likes with ${element.matchProfile.firstName} | ID : ${element.matchProfile.userId}`)
                                await doMatched(element.matchProfile.userId, element.matchProfile.firstName)
                            }
                        })
                    }
                })
